	...
	: GameObject(GameObjectBuilder().Build())
{
	...
	// 一つ目のウィンドウ
	RectDetectorConfig firstConfig = {.base =
										  {
...
void HighlightEnemy::Update()
{
	...
	/*for (const auto& target : secondWndRectDetector_.detectedTargets)
	{
		MTImGui::Instance().TypedShow(&secondWndRectDetector_.detectedTargets, "SecondWndRectDetector",
	ShowType::Inspector);
	}*/
...
void HighlightEnemy::Draw() const
{
	...
	firstWndRectDetector_.ForEach(
		[this, ratio](const ScreenCoordContainsInfo& _info)
		{
			if (_info.screenPos.z > 0.0f && _info.screenPos.z < 1.0f)
			{
				RectF targetRect{(_info.screenPos.x - highlightFrameSize.x * ratio.x / 2.0f),
