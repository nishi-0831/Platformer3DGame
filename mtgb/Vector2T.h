...
namespace mtgb
{
	...
	template <Arithmetic T> struct Vector2T
	{
		T x;
		T y;
...
#pragma region 四則演算
		...
		inline Vector2T& operator*=(const Vector2T& _other)
		{
			x *= _other.x;
			y *= _other.y;
			return *this;
...
#pragma region グローバルオペレータ
	...
	template <typename T, typename U>
	inline auto operator*(U _scalar, const Vector2T<T>& _v) -> Vector2T<std::common_type_t<T, U>>
	{
		return _v * _scalar;
	}
